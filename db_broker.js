...
module.exports.SelectById = (id) => {
    return new Promise((resolve, reject) => {
        pool.query("SELECT * FROM todolist WHERE (idtodolist = ?)", id, (err, results) =>{
            err ? reject(err) : resolve(results)
        })
    })
}
...
//this method is similar to findBy, but it looks for the status of a specific item in the to-do list
module.exports.getStatus = (id) => {
    return new Promise((resolve, reject) => {
        pool.query("SELECT status FROM todolist WHERE (idtodolist = ?)", id, (err, results) => {
            err ? reject(err) : resolve(results)
        })
    })
}
